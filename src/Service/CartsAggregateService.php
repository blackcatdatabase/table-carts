...
namespace BlackCat\Database\Packages\Carts\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\Carts\Dto\CartDto;
use BlackCat\Database\Packages\Carts\Mapper\CartDtoMapper;
 ...
 */
final class CartsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private CartRepository $cartRepo
    ) {
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\Carts\ModuleException("Record {$idStr} not found for locking.");
            }
