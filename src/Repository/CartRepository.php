...
use BlackCat\Database\Packages\Carts\Repository\CartRepositoryInterface;
...
class CartRepository implements CartRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'id' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Carts\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'user_id', 'note', 'updated_at', 'version' ]);
          return count($rows);
        }
