...
namespace BlackCat\Database\Packages\Carts\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Carts\Definitions;
use BlackCat\Database\Packages\Carts\Criteria;
use BlackCat\Database\Packages\Carts\Dto\CartDto as Dto;
use BlackCat\Database\Packages\Carts\Mapper\CartDtoMapper as RowMapper;
...
final class CartRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Carts\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'user_id', 'note', 'updated_at', 'version' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Carts\Definitions::table());
            return $bulk->upsertMany($rows, [ 'id' ], [ 'user_id', 'note', 'updated_at', 'version' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Carts\Dto\CartDto|null */
    public function getByTenantIdAndId(int $tenantId, string $id, bool $asDto = false): array|\BlackCat\Database\Packages\Carts\Dto\CartDto|null {
        $row = $this->getByUnique([ 'tenant_id' => $tenantId, 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Carts\Mapper\CartDtoMapper::fromRow($row);
    }
    public function existsByTenantIdAndId(int $tenantId, string $id): bool {
    ...
    public function getIdByTenantIdAndId(int $tenantId, string $id) {
        $row = $this->getByTenantIdAndId($tenantId, $id);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Carts\Dto\CartDto|null */
    public function getById(string $id, bool $asDto = false): array|\BlackCat\Database\Packages\Carts\Dto\CartDto|null {
    ...
    /** @return int|string|null */
    public function getIdById(string $id) {
        $row = $this->getById($id);
        return $row ? ($row['id'] ?? null) : null;
    }

    /** @return array<string,mixed>|\BlackCat\Database\Packages\Carts\Dto\CartDto|null */
    public function getByIdForTenant(int|string $id, int|string $tenantId, bool $asDto = false): array|\BlackCat\Database\Packages\Carts\Dto\CartDto|null {
